hooks:add('onInitDone', 'load_physics', function()
	...
	local function findWaterBlockToRemove(world, x, y, z, dirx, dirz)
		...
		while true do
			-- Check up
			if world:getBlockUnsafe(x, y + 1, z) == 8 then
				y = y + 1

			-- Check up forward
			elseif world:getBlockUnsafe(x + 1, y + 1, z) == 8 then
				x = x + 1
				y = y + 1
			...
			-- Check up left
			elseif world:getBlockUnsafe(x, y + 1, z + 1) == 8 then
				z = z + 1
				y = y + 1
			...
			-- Check forward
			elseif dirx >= 0 and world:getBlockUnsafe(x + 1, y, z) == 8 then
				dirx = 1
				x = x + 1
			...
			-- Check left
			elseif dirz >= 0 and world:getBlockUnsafe(x, y, z + 1) == 8 then
				dirz = 1
				z = z + 1
	...
	local function findWaterBlockToCreate(world, x, y, z)
		...
		-- Under
		if world:getBlockUnsafe(x, y - 1, z) == 0 then
			return x, y - 1, z
		end
		...
		-- nearest x
		for dx = -1, 1, 2 do
			if world:getBlockUnsafe(x + dx, y, z) == 0 then
				dirX = dirX + dx
				if dirX == 0 then
					dirX = math.random(0, 1) * 2 - 1
				end

				if world:getBlockUnsafe(x+dx, y - 1, z) == 0 then
					return x + dx, y - 1, z
				end
			end
		end
		...
		-- nearest y
		for dz = -1, 1, 2 do
			if world:getBlockUnsafe(x, y, z + dz) == 0 then
				dirZ = dirZ + dz
				if dirZ == 0 then
					dirZ = math.random(0, 1) * 2 - 1
				end

				if world:getBlockUnsafe(x, y - 1, z + dz) == 0 then
					return x, y - 1, z+dz
				end
			end
		end
		...
		if dirX > 0 then
			for dx = 2, WATER_LEAK_SIZE do
				if world:getBlockUnsafe(x + dx, y, z) ~= 0 then
					limiterX = dx - 1
					break
				elseif world:getBlockUnsafe(x + dx, y - 1, z) == 0 then
					return x + 1, y, z
				end
			end
		end
		...
		if dirZ > 0 then
			for dz = 2, WATER_LEAK_SIZE do
				if world:getBlockUnsafe(x, y, z + dz) ~= 0 then
					limiterZ = dz - 1
					break
				elseif world:getBlockUnsafe(x, y - 1, z + dz) == 0 then
					return x, y, z + 1
				end
			end
		end
			...
			for dx = 1, limiterX do
				...
				if dirZ > 0 then
					for dz = 1, limiterZ do
						if world:getBlockUnsafe(x + dx, y, z + dz) ~= 0 then
							break
						end
						if world:getBlockUnsafe(x + dx, y - 1, z + dz) == 0 then
							if world:getBlockUnsafe(x + 1, y - 1, z) then
								return x + 1, y, z
							else
								return x, y, z + 1
							end
			...
			for dx = 1, limiterX do
				...
				if dirZ > 0 then
					for dz = 1, limiterZ do
						...
						if world:getBlockUnsafe(x - dx, y - 1, z + dz) then
							if world:getBlockUnsafe(x - 1, y - 1, z) then
								return x - 1, y, z
							else
								return x, y, z + 1
